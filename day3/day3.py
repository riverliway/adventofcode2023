...
def flood_search(grid, x, y, check_num, flood_num):
  """
  If this index is a 2, mark it as 3 along with all other 2s touching on the same row
  """
  if x < 0 or y < 0 or x >= len(grid[0]) or y >= len(grid) or grid[y][x] != check_num:
    return
  
  grid[y][x] = flood_num
  flood_search(grid, x + 1, y, check_num, flood_num)
  flood_search(grid, x - 1, y, check_num, flood_num)

def main1():
  engine_grid = input.split('\n')
  valid_grid = [[0 for _ in engine_grid[0]] for _ in engine_grid]

  # Turns valid_grid into a grid of 0 (for empty), 1 (for symbols), and 2 (for part number)
  for (line_index, line) in enumerate(engine_grid):
    for (char_index, char) in enumerate(line):
      if char.isdigit():
        valid_grid[line_index][char_index] = 2
      elif char != '.':
        valid_grid[line_index][char_index] = 1

  # Flood searches any part number adjacent to a symbol with 3
  for (line_index, line) in enumerate(valid_grid):
    for (char_index, char) in enumerate(line):
      if char == 1:
        flood_search(valid_grid, char_index + 1, line_index, 2, 3)
        flood_search(valid_grid, char_index - 1, line_index, 2, 3)
        flood_search(valid_grid, char_index + 1, line_index + 1, 2, 3)
        flood_search(valid_grid, char_index - 1, line_index - 1, 2, 3)
        flood_search(valid_grid, char_index - 1, line_index + 1, 2, 3)
        flood_search(valid_grid, char_index + 1, line_index - 1, 2, 3)
        flood_search(valid_grid, char_index, line_index + 1, 2, 3)
        flood_search(valid_grid, char_index, line_index - 1, 2, 3)

  # Any part number that is not a 3 is invalid
  for (line_index, line) in enumerate(valid_grid):
    for (char_index, char) in enumerate(line):
      if valid_grid[line_index][char_index] != 3:
        engine_grid[line_index] = engine_grid[line_index][:char_index] + '.' + engine_grid[line_index][char_index + 1:]

  ssum = 0
  for line in engine_grid:
    ss = [int(c) for c in line.split('.') if c != '']
    ssum += sum(ss)

  print(ssum)

def find_start_end(grid, x, y, num) -> tuple[int]:
  """
  Finds the start and end xs of a part number
  """
  start = x
  end = x

  while start >= 0 and grid[y][start] == num:
    start -= 1

  while end < len(grid[0]) and grid[y][end] == num:
    end += 1
...
def main2():
  ...
  # Turns valid_grid into a grid of 0 (for empty), 1 (for gears), and 2 (for part number)
  for (line_index, line) in enumerate(engine_grid):
    for (char_index, char) in enumerate(line):
      if char.isdigit():
        valid_grid[line_index][char_index] = 2
  ...
  # Flood searches any part number adjacent to a gear with 3
  for (line_index, line) in enumerate(valid_grid):
    for (char_index, char) in enumerate(line):
      if char == 1:
        flood_search(valid_grid, char_index + 1, line_index, 2, 3)
      ...
      if char == 1:
        for x in [-1, 0, 1]:
          for y in [-1, 0, 1]:
            if x == 0 and y == 0:
              continue
            if valid_grid[line_index + y][char_index + x] == 3:
              flood_search(valid_grid, char_index + x, line_index + y, 3, 4)
              start_index, end_index = find_start_end(valid_grid, char_index + x, line_index + y, 4)
              adjacents.append(int(engine_grid[line_index + y][start_index:end_index + 1]))
